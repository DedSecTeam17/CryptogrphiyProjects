...
public class PlayFair_Encryption_Controller  implements  Initializable{
        ...
        public void initialize(URL location, ResourceBundle resources) {
            ...
            play_fair_save_edited_file.setOnAction(event -> {
                try {
                    play_fair_view_model.getInstance().saveEditOnFile( play_fair_file_result,messageFilePath);
            ...
            play_fair_delete_file_contetn.setOnAction(event -> {
                try {
                    play_fair_view_model.getInstance().deleteAllFileContent(messageFilePath, play_fair_file_result);
                    messageFromFile="";
                } catch (IOException e) {
                    e.printStackTrace();
                }
            ...
            play_fair_encry.setOnAction(event -> {
                play_fair_view_model.getInstance().bindRealEncryption( play_fair_message, play_fair_key);
            });
            play_fair_decry.setOnAction(event ->{
                        play_fair_view_model.getInstance().bindRealDecryption( play_fair_message, play_fair_key);

                    }
            );
            play_fair_upload_file.setOnAction(event -> {
                try {
                    Map<String,String> file_meta_data=   play_fair_view_model.getInstance().bindUploadFile( play_fair_file_result,file_path);
            ...
            play_fair_encry_file.setOnAction(event -> {
                try {
                    this.messageFromFile=   play_fair_view_model.getInstance().bindFileEncryption(messageFromFile,messageFilePath, play_fair_file_result, play_fair_files_key);
//                    play_fair_view_model.getInstance().bindFileEncryption(messageFromFile,messageFilePath, play_fair_file_result, play_fair_files_key);
//                    this.messageFromFile=   play_fair_view_model.getInstance().bindFileEncryption(messageFromFile,messageFilePath, play_fair_file_result, play_fair_files_key);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            ...
            play_fair_decry_file.setOnAction(event -> {
                try {
                    this.messageFromFile=   play_fair_view_model.getInstance().bindFileDecryption(messageFromFile,messageFilePath, play_fair_file_result, play_fair_files_key);
//                    play_fair_view_model.getInstance().bindFileDecryption(messageFromFile,messageFilePath, play_fair_file_result, play_fair_files_key);
//                    this.messageFromFile=   play_fair_view_model.getInstance().bindFileDecryption(messageFromFile,messageFilePath, play_fair_file_result, play_fair_files_key);
                } catch (IOException e) {
                    e.printStackTrace();
                }
